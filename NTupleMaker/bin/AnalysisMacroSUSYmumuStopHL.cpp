...
//				cout<<" reading now  "<<iEntry<<endl;
	...
	float relIso = analysisTree.muon_relIso[im];	
	...
	for (unsigned int im1=0; im1<isoMuons.size(); ++im1) {
	  ...
	  for (unsigned int im2=im1+1; im2<isoMuons.size(); ++im2) {
	    ...
	    if (dRmumu>dRleptonsCut) {
	      //double sumIso = analysisTree.muon_pt[im1]+analysisTree.muon_pt[im2];
...
//	if (event_thirdLeptonVeto && foundThirdLepton>1) continue;
	...
	mu_relIsoMu[im]  = analysisTree.muon_relIso[mIndex] ;
      ...
      mu_relIso[0]  = analysisTree.muon_relIso[muon_index_1] ;
      mu_relIso[1]  = analysisTree.muon_relIso[muon_index_2] ;
