...
namespace{
  int Nsmooth = 2; // Number of times to smooth TH2D
...
int main(int argc, char *argv[]){
 ...
 //   else 
    ...
    /*
   vobs.push_back(pexp);
    vobsup.push_back(pup);
    vobsdown.push_back(pdown);
  */
    vexp.push_back(pexp);
    vup.push_back(pup);
  ...
  TString xparticle("gluino"), yparticle("#tilde{#chi}_{1}^{0}");
  if(model=="T2tt") xparticle = "stop";
  if(model=="stau") xparticle = "#tau";
  if(model=="stau-stau") xparticle = "#tau";
  if(model=="C1N2") xparticle = "#tilde{#chi}_{1}^{#pm}/#tilde{#chi}_{2}^{0}";
  if(model=="ChiWZ") xparticle = "#tilde{#chi}_{1}^{#pm}/#tilde{#chi}_{2}^{0}";
  if(model=="ChiWH") xparticle = "#tilde{#chi}_{1}^{#pm}/#tilde{#chi}_{2}^{0}";
  if(model=="C1C1") xparticle = "#tilde{#chi}_{1}^{#pm}";
  if(model=="T6ttWW") {
    xparticle = "sbottom";
    yparticle = "chargino";
  }
...
TGraph DrawContours(TGraph2D &g2, int color, int style,
  ...
  if(Nsmooth>0){
    g2.SetNpx(120);
    g2.SetNpy(80);
  if(model=="C1N2") g2.SetNpx(500);
  if(model=="C1N2" ) g2.SetNpy(100);

  if(model=="C1N2x0p05") g2.SetNpx(480);
  if(model=="C1N2x0p05" ) g2.SetNpy(60);
  
  if(model=="C1N2x0p95") g2.SetNpx(100);
  if(model=="C1N2x0p95") g2.SetNpy(150);
  
  if(model=="C1C1") g2.SetNpx(75);
 //if(model=="C1C1") g2.SetNpy(100);

  if(model=="ChiWH") g2.SetNpy(80);
...
//    TH2D *hclone = static_cast<TH2D*>(histo2d->Clone("clone"));
  ...
  //  double glu_lsp = 1;
	...
	//if (model =="C1C1 " && x-y<50)
	//histo2d->SetBinContent(binx,biny,2);
	//bool bpoint=false;
//	if (x>300 && x<325 && y>170 )cout<<" x "<< x  <<" y  "<<y<<" z "<<z<<endl;
////	if (x-y <150 ) bpoint=false;
	vx.push_back(x);
	vy.push_back(y);
//	if (model == "C1C1 " && x-y<75)
	vz.push_back(z);
...
//	}
	//else if(y>200) vz.push_back(1.);
//	cout<<"  "<<x<<"  "<<y<<"  "<<hclone->GetBinContent(hclone->GetBin(binx,biny))<<endl;
      }
    }
 ...
 /* for(int point=0; point < n_points; point++){
    g->GetPoint(point, mGLU, mLSP);
    if (mGLU-mLSP<125 && point>1.0) g->RemovePoint(point);
    }
*/
...
void fixGraph(TGraph &graph){
  //double glu_lsp = 300;
  int np(graph.GetN());
  double mglu, mlsp,iniglu, endglu, inilsp, endlsp;
  //cout<<endl<<endl;

  graph.GetPoint(0, iniglu, inilsp);
  graph.GetPoint(np-1, endglu, endlsp);
 // cout<<"  "<<endglu<<"  "<<endlsp<<endl;
  ...
  graph.GetPoint(point, mglu, mlsp);
    if(mglu-mlsp <150 && model == "ChiWH" ){
cout<<" this should be removed ?mglu  "<<mglu <<"  "<<mlsp<<"  "<<point<<endl;
      //while(point <= graph.GetN() && point!=0) {
      while(point <= graph.GetN() ) {
	graph.RemovePoint(graph.GetN()-1);
    	graph.SetPoint(graph.GetN()-1, mglu-50, mglu-175);
	np--;
      }
//      break;
    }
    if(mglu-mlsp <51 && mglu<300 && model == "C1N2" ){
cout<<" this should be removed mglu  "<<mglu <<"  "<<mlsp<<"  "<<point<<"  model "<<model<<endl;
      while(point <= graph.GetN() && point!=0) {
   //   while(point <= graph.GetN() ) {
	graph.RemovePoint(graph.GetN()-1);
//    	graph.SetPoint(graph.GetN()-1, mglu-50, mglu-175);
...
//      break;
    ...
    if(mglu >90 && (mglu-mlsp <50  || mlsp>=mglu+25) && model == "C1C1"  && np>0){
cout<<" this should be removed mglu  "<<mglu <<"  "<<mlsp<<"  "<<point<<"  model "<<model<<"  "<<graph.GetTitle()<<endl;
     //while(point <= graph.GetN() && point!=0) {
      while(point < graph.GetN() && point>0) {
	graph.RemovePoint(graph.GetN()-1);
	np--;
      }
    	graph.SetPoint(graph.GetN()-1, mglu-5, mlsp-15);
 //    break;
    }
