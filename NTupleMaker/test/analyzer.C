...
void analyzer::Loop(){
	...
	TFile * filein = new TFile("/nfs/dust/cms/user/alkaloge/TauAnalysis/new/new/CMSSW_8_0_12/src/DesyTauAnalyses/NTupleMaker/test/25ns/FILEIN","read");
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) {
		...
		if ( jentry%100000 ==0 ) cout<<" working for you...processed " <<jentry<<" events from "<<nentries<<endl;
...
//		cout<<" e " <<jentry<<" from "<<nentries<<"  weights "<<all_weight<<" same ? "<< gen_weight*trig_weight*pu_weight*top_weight*LSF_weight*TFR_weight <<" PU "<<pu_weight<<" top "<<top_weight<<" LSF "<<LSF_weight<<" TFR "<<TFR_weight<<" gen "<<gen_weight<<" trig  "<<trig_weight<<endl;
		int iCut = 2;
...
/*
		bool cut1 = if (countjets>0);
		bool cut2 = if (RhT < 1.);
		bool cut3 = if (PtOHT<2.);
...
*/
		...
		iCut++;
		 ...
		 for(int ci = 0; ci < CutNumb; ci++)
					...
					cout<<" Count "<<ci<<" binContent "<<CutFlow->GetBinContent(1+ci)<<"  CFCounter_ "<<CFCounter_[ci]<<" CFCounter "<<CFCounter[ci]<<" iCFCounter "<<iCFCounter[ci]<<" Name "<<CutFlowUnW->GetXaxis()->GetBinLabel(1+ci)<<" entry  "<<CutFlowUnW->GetBinContent(1+ci)<<"  weight "<<all_weight<<endl;			
}
