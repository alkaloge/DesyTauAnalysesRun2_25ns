...
//cout<<"  "<<DiL.M()<<"  "<<CutIndex<<endl;
 ...
 //   hdRme_dPhi[CutIndex]->Fill(Dr,dPhi,EvWeight);
  ...
  if (JetsV.size()>0){
    ...
    //for (unsigned int ij=0;ij<jet_count;ij++){
...
/*
    for (unsigned int ij=0;ij<jet_count;ij++){
	    ...
	    for (int in = 0 ; in<btag_index.size();in++){
	    if (jet_btag[ij] > 0.89)   
	 {
		 bjets++;
	//cout<<" Jet pT from .h "<<ij<<"  "<<jet_pt[ij]<<endl;
	 }
	    }
	 
    }*/
	//	cout<<"  " <<btag_index.size()<<endl;
    bjets = btag_index.size();
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      ...
      //chain->Add("25ns/TTPowHeg_B_OS.root/mutau/T");

      chain->Add("/nfs/dust/cms/user/alkaloge/TauAnalysis/new/new/CMSSW_8_0_12/src/DesyTauAnalyses/NTupleMaker/test/25ns/FILEIN/mutau/T");
      //chain->Add("25ns/SingleMuon_Run2015D_05Oct2015_601_B_OS_DataDriven.root/mutau/T");
      //chain->Add("25ns/SingleMuon_Run2015D_PRv4_349_B_OS_DataDriven.root/mutau/T");
      tree = chain;
#endif // SINGLE_TREE
