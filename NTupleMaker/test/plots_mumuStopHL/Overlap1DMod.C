...
const int mycolorqcd=TColor::GetColor("#C390D4");
...
TH1::SetDefaultSumw2();
	...
	string channel="mumuStopHL";
	string dirr="/afs/cern.ch/user/a/alkaloge/work/HLLHC/CMSSW_8_0_29/src/DesyTauAnalyses/NTupleMaker/test/plots_"+channel+"/";
	dirr="";
	string line;
...
*/
	...
	for (int ct=0;ct<cuts.size();ct++){

	sprintf(varr,"%s%s",variables[vr].c_str(),cuts[ct].c_str());
	//variable=variables[vr].c_str()+cuts[ct].c_str();
	variable=varr;
...
//	}
	...
	TH1D* allbkg, *htt,*hstop,*hwj,*hdyj,*hztt,*hdib, *hww,*hfakes,*httx,*hrest,*hqcd;
	THStack *hs;
	...
	int count=0;
		...
		if (obj->IsA ()->InheritsFrom ("TH1") ) {
			...
			cout<<" "<<fname<<eventCountt->GetEntries()<<"  "<<xsecc<<"  "<<float(eventCountt->GetEntries()/xsecc)<<endl;
...
////////////////hrest includes single-top VV
                ...
                if (std::string::npos != title_.find("ST_") || std::string::npos != title_.find("channel") )  {col= mycolorst; hstop->Add(h2);hstop->SetLineColor(col);}
                        if ( std::string::npos != title_.find("VV") ) {col=mycolorvv; hdib->Add(h2); hdib->SetLineColor(col);}
                        if ( std::string::npos != title_.find("WWW") || std::string::npos != title_.find("WZ")  || std::string::npos != title_.find("ZZZ")  ) {col=mycolorww; hww->Add(h2); hww->SetLineColor(col);}
...
//				cout<<"  sign "<<sing<<" nb "<<nbb<<endl;
			...
			for (int nb=1;nb<= hh[1]->GetNbinsX();nb++){
				...
				cout<<" significance  Bin "<<nb<<"  "<<float(hh[2]->GetBinContent(nb) /sqrt(allnew_->GetBinContent(nb)))<<endl;
			}
