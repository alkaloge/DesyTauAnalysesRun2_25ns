...
if (SO_)
	...
	/////////////SinglTop
		...
		if (obj->IsA ()->InheritsFrom ("TH1")) {
			...
			float v1f = htt->GetSumOfWeights()+hstop->GetSumOfWeights();
			float v2f= hdyj->GetSumOfWeights();
			float v3f=hwj->GetSumOfWeights();
			float v4f=hdib->GetSumOfWeights()+hww->GetSumOfWeights()+httx->GetSumOfWeights()+hqcd->GetSumOfWeights();
			float v5f=hztt->GetSumOfWeights();
			vals[0]= v1f;
			vals[1]= v2f;
			vals[2]= v3f;
			vals[3]= v4f;
			...
			Int_t colors[] = { htt->GetLineColor(),hdyj->GetLineColor(),hwj->GetLineColor(),httx->GetLineColor(),hztt->GetLineColor()};
			...
			pie3->GetSlice(1)->SetTitle("DY#rightarrow ll");
			pie3->GetSlice(2)->SetTitle("WJets");
			pie3->GetSlice(3)->SetTitle("Rest");
			pie3->GetSlice(4)->SetTitle("DY#rightarrow#tau#tau");
			...
			sprintf(savename,"Plots/Pie%s.pdf",nan_.c_str());
...
if (SO_){
				...
				v2->SetLineColor(mycolorztt);v2->SetFillColor(mycolorztt);legend_c1->AddEntry(v2, "DY#rightarrow#tau#tau", "f");
				...
				v7->SetLineColor(mycolorttx);v7->SetFillColor(mycolorttx);legend_c1->AddEntry(v7, "TTXJets", "f");
				...
				v8->SetLineColor(mycolorvv);v8->SetFillColor(mycolorvv);legend_c1->AddEntry(v8, "VV(V)", "f");
			...
			sprintf(nnn,"Entries  / %d %s",hh[1]->GetBinWidth(2),units);
			...
			sprintf(nnn,"Entries  / %.2f %s",hh[1]->GetBinWidth(2),units);
...
*/
	...
	if(!WDD){
			if (SO_ && !QCDMC)sprintf(f,"Plots%sSO.pdf",namee);
			if (SO_ && QCDMC)sprintf(f,"Plots/%s_QCDMC_SO.pdf",namee);
			if(!SO_ && !QCDMC) sprintf(f,"Plots/%s.pdf",namee);
			if(!SO_ && QCDMC) sprintf(f,"Plots/%s_QCDMC.pdf",namee);
		}
		if (WDD){
			if (SO_ && !QCDMC)sprintf(f,"Plots/%sSO.pdf",namee);
			if (SO_ && QCDMC)sprintf(f,"Plots/%s_QCDMC_SO.pdf",namee);
