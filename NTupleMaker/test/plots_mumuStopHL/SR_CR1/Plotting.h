...
void DrawCMSLogo(TPad* pad, TString cmsText, TString extraText, int iPosX,
  ...
  if (outOfFrame) {
    ...
    latex.DrawLatex(l, 1 - t + lumiTextOffset * t +0.085, cmsText);
  }
  ...
  if (!outOfFrame) {
    ...
    latex.DrawLatex(posX_, posY_+0.07, cmsText);
    if (writeExtraText) {
      latex.SetTextFont(extraTextFont);
      latex.SetTextAlign(align_);
      latex.SetTextSize(extraTextSize * t * pad_ratio);
     // latex.DrawLatex(posX_, posY_ - relExtraDY * cmsTextSize * t, extraText);
      latex.DrawLatex(posX_+0.085, posY_+0.062 , extraText);
    }
...
void FixTopRange(TPad *pad, double fix_y, double fraction) {
  ...
  if (gPad->GetLogy()) {
    ...
    max = std::pow(8, max);
    hobj->SetMaximum(max);
    hobj->GetYaxis()->SetNdivisions(606,false);
  }
}
