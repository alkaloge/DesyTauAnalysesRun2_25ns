...
TH1::SetDefaultSumw2();
	...
	vector<float> xsecs_;
	signal_names.clear();
	ifstream ifs("datasets2D_Stop_"+reg);
	string channel="mumuStopMC";
...
*/		
		else if (std::string::npos == dataname.find("Stop")  && (syst=="METUp" || syst=="METDown"  || syst=="EWKUp" || syst=="EWKDown") ) {
		...
		//	cout<<" Non signal filename and syst METUp or METDown "<<endl;
			...
			titles.push_back(dataname+"_B.root");
				...
				(std::string::npos != dataname.find("Stop")  || std::string::npos != dataname.find("stau")) ) {
...
*/			
	...
	/*
	variables.push_back("met_Smin_16");
	variables.push_back("met_MTtot_16");
	variables.push_back("met_MTDil_16");
	variables.push_back("met_MTDil2_16");
	variables.push_back("met_Smin_17");
	variables.push_back("met_MTtot_17");
	variables.push_back("met_MTDil_17");
	variables.push_back("met_MTDil2_17");
	variables.push_back("met_Smin_18");
	*/
	variables.push_back("met_MTDil_15");
	variables.push_back("met_MTDil2_15");
	variables.push_back("met_MTDil3_15");
	variables.push_back("met_MTDil4_15");
	variables.push_back("met_MTDil5_15");
	variables.push_back("met_MTDil6_15");
	variables.push_back("met_MTDil7_15");
	variables.push_back("met_MTDil8_15");
...
////////////////hrest includes single-top VV
			...
			if (std::string::npos != title_.find("DataDriven") || std::string::npos != title_.find("QCD"))  {col= mycolorfakes;hfakes->Add(h2,1); hfakes->SetLineColor(col); }
	...
	if(b_scale)		allbkg->Scale(scale);
			...
			/*
				if (variable=="met_MTDil_19")  {for (int k=1;k<19;k++) keep_bin.push_back(k);label_bin.push_back("lab");}
				if (variable=="met_MTDil2_19")  {for (int k=1;k<11;k++) keep_bin.push_back(k);}
				if (variable=="met_MTDil3_19")  {for (int k=1;k<21;k++) keep_bin.push_back(k);}
				*/
...
//				cout<<"  sign "<<sing<<" nb "<<nbb<<endl;
			...
			for (int nb=1;nb<= hh[1]->GetNbinsX();nb++){

				if (allnew_->GetBinContent(nb)>0)
				cout<<" significance  Bin "<<nb<<"  "<<float(hh[3]->GetBinContent(nb) /sqrt(allnew_->GetBinContent(nb)))<<endl;
			}
...
ModifyHist (TH1D* &h, int cl_ ,float & lumi,float & weight,string & title_, bool norm_=false)
{
	...
	int col=cl_;
			...
			if ( std::string::npos != title_.find("tau") || std::string::npos != title_.find("Stop") || std::string::npos != title_.find("SMS")){
