...
//int mycolorztt=TColor::GetColor("#00cccc");
...
//	TCanvas *c1 = new TCanvas ("c1","c1",0,22,600,600);//
	...
	while ((key = (TKey *) nextkey ())) {
		...
		//if (string::npos == nn.find("Mt2lestermutau") ) {MaxEventsBin = -1;drawData = true;  flcont=false;};
		//if (string::npos == nn.find("Iso") ) {MaxEventsBin = -1;drawData = true; };
		//if (string::npos == nn.find("Mode") ) {MaxEventsBin = -1;drawData = false;  flcont=false;};
		if (string::npos == nn.find("_18") ) flcont=false;
		//if (string::npos == nn.find("Middle") ) continue;
		if (string::npos != nn.find("DeltaMET") ) continue;
		if (string::npos != nn.find("ByBin") ) continue;
		//if (string::npos == nn.find("nJet_17")) flcont=false;
		if (!flcont) continue;
...
if (SO_) legend_c1    = new TLegend (0.45, 0.92, 0.95, 0.65);
...
if (SO_)
	...
	//					cout<< " for "<<fname<<" file, there xsec is "<<xsecc<<" SuOfWeights "<<"  "<<h2->GetSumOfWeights()<<" Lumi "<<Lumi<<" lumiweight "<<lumiweights[cl-1]<<" weight "<<float(xsecc*Lumi/nGenn)<<endl;
			...
			int col=1;
			//////////TT
			if (std::string::npos != title_.find("TT_TuneCUETP8M2T4_13TeV-powheg-pythia8") || std::string::npos != title_.find("TTJets")) { col= mycolortt;htt->Add(h2); htt->SetLineColor(col) ;}
			///////////Fakes
			if (std::string::npos != title_.find("Fakes"))  {col= mycolorqcd; hqcd->Add(h2); hqcd->SetLineColor(col); }
			///////////////Zll
	...
	/////////////SinglTop
			...
			if ( std::string::npos != title_.find("TTW") || std::string::npos != title_.find("TTZ") || std::string::npos != title_.find("TG") || std::string::npos != title_.find("tG") || std::string::npos != title_.find("TTG") || std::string::npos != title_.find("ttW") || std::string::npos != title_.find("tZ") || std::string::npos != title_.find("TTT_") ) {col=mycolorttx ;httx->Add(h2);   httx->SetLineColor(col);}
		...
		if (obj->IsA ()->InheritsFrom ("TH1")) {
			...
			float vals[]={0,0,0};
			float v1f = htt->GetSumOfWeights();
			float v2f= hdib->GetSumOfWeights()+hdyj->GetSumOfWeights()+hwj->GetSumOfWeights()+hww->GetSumOfWeights();
			float v3f=httx->GetSumOfWeights()+hstop->GetSumOfWeights();
			vals[0]= v1f;
			vals[1]= v2f;
			vals[2]= v3f;
			...
			Int_t colors[] = { hqcd->GetLineColor(),hdib->GetLineColor(),hdyj->GetLineColor()};
			...
			pie3->SetTextAngle(0.90);
			pie3->SetTextSize(0.035);
			...
			//pie3->GetSlice(0)->SetTitle(lumitag);
                        //pie3->SetLabelFormat("#splitline{%txt}{%perc}");
                        //pie3->SetLabelFormat("%txt-%perc");
			pie3->GetSlice(0)->SetTitle("TTJets");
			pie3->GetSlice(1)->SetTitle("Rest");
	//		pie3->GetSlice(2)->SetTitle("WW");
	//		pie3->GetSlice(3)->SetTitle("t#bar{t}+SingleT");
			pie3->GetSlice(2)->SetTitle("TTX+singleT");
			...
			//pie3->Draw("3d");
			nan_.erase(0,4);
			sprintf(savename,"Shapes/Pie%s.pdf",nan_);
			...
			pad1->SetLogy();
			 if (std::string::npos != nn.find("Mt2lester")  || std::string::npos != nn.find("MET_") || std::string::npos != nn.find("HighDzeta_") ||  std::string::npos != nn.find("MiddleDzeta_") ||  std::string::npos != nn.find("LowDzeta_"))	{ pad1->SetLogx(); }
			//pad1->SetLogx();
			//pad1->SetGridx();
			pad1->Draw();
...
if (SO_){
   				...
   				legend_c1->AddEntry(hh[3],"#tilde{t}(200), #tilde{#chi}_{1}^{0}(25)","l");
   				legend_c1->AddEntry(hh[4],"#tilde{t}(400), #tilde{#chi}_{1}^{0}(1)","l");
   				legend_c1->AddEntry(hh[5],"#tilde{t}(600), #tilde{#chi}_{1}^{0}(325)","l");

}

				v1->SetLineColor(kRed);v1->SetFillColor(kRed);legend_c1->AddEntry(v1, "Bkg", "f");
	...
	//			v10->SetLineColor(kBlack);legend_c2->AddEntry(hh[1], lab_, "l");
			...
			sprintf(nnn,"Entries  / %d %s",hh[1]->GetBinWidth(2),units.c_str());

			if  (std::string::npos != titlee.find("dz") || std::string::npos != titlee.find("dxy") || std::string::npos != titlee.find("Phi") || std::string::npos != titlee.find("phi") || std::string::npos != titlee.find("dR") )
			sprintf(nnn,"Entries  / %.2f %s",hh[1]->GetBinWidth(2),units.c_str());


			if ( (std::string::npos != titlee.find("nJ") || std::string::npos != titlee.find("nB") ||  std::string::npos != titlee.find("nEl") || std::string::npos != titlee.find("nM") || std::string::npos != titlee.find("nT") ) && std::string::npos == titlee.find("DZeta_") )
			sprintf(nnn,"Entries");

			if ( std::string::npos != titlee.find("dEta") ||  std::string::npos != titlee.find("Eta") || std::string::npos != titlee.find("eta")  )                         sprintf(nnn,"Entries  / %.f",hh[1]->GetBinWidth(2));

			if ((std::string::npos != titlee.find("DZeta_") || std::string::npos != titlee.find("MET_") ||  std::string::npos != titlee.find("Mt2lester") ||  std::string::npos != titlee.find("Low") || std::string::npos != titlee.find("Middle") || std::string::npos != titlee.find("High") ) && std::string::npos == titlee.find("FB_") && std::string::npos == titlee.find("Cos") && std::string::npos == titlee.find("Phi") && std::string::npos == titlee.find("phi"))
			sprintf(nnn,"Entries / bin");


			hsum->GetYaxis()->SetTitle(nnn);
			allbkg->GetYaxis()->SetTitle(nnn);

			if (norm_) {hsum->Scale(1/hsum->Integral()); hsum->SetFillColor(0);hsum->SetLineColor(kRed);}
			if (norm_) {allbkg->Scale(1/allbkg->Integral()); allbkg->SetFillColor(0);allbkg->SetLineColor(kRed); allbkg->SetLineStyle(1);}

			allbkg->SetMinimum(0.0015);
			allbkg->Draw("hist");
...
if (SO_){
			for (int ij=2;ij<6;ij++){
				...
				hh[ij]->SetFillStyle(3004);
				hh[ij]->GetYaxis()->SetTitle(nnn);
				hh[ij]->Draw("hist same");
			}
}

			allbkg->SetMaximum(50 * max(allbkg->GetMaximum(),hh[2]->GetMaximum()));
			...
			if (SO_ && !QCDMC)sprintf(f,"Shapes/%sSO_PDF.pdf",namee);
			if (SO_ && QCDMC)sprintf(f,"Shapes/%s_QCDMC_SO_PDF.pdf",namee);
			if(!SO_ && !QCDMC) sprintf(f,"Shapes/%s_PDF.pdf",namee);
			if(!SO_ && QCDMC) sprintf(f,"Shapes/%s_QCDMC_PDF.pdf",namee);
...
void ModifyHist (TH1D* &h, int cl_ ,float & lumi,float & weight,string & title_, bool norm_=false,float XSec=1.)
		{
			...
			if (std::string::npos != title_.find("TT_TuneCUETP8M2T4_13TeV-powheg-pythia8") || std::string::npos != title_.find("TTJets")) { col= mycolortt; }

			if (std::string::npos != title_.find("Fakes"))  {col= mycolorqcd; }
			...
			if ( std::string::npos != title_.find("TTW") || std::string::npos != title_.find("TTZ") || std::string::npos != title_.find("tZ") || std::string::npos != title_.find("TG") || std::string::npos != title_.find("tG")  || std::string::npos != title_.find("ttW") || std::string::npos != title_.find("TTT_") ) {col=mycolorttx ;}
		...
		//	if (std::string::npos != title1.find("pT"))  h->GetXaxis()->SetTitle("GeV/c^{2}");
			...
			if (std::string::npos != title1.find("dEtaDil"))  h->GetXaxis()->SetTitle("#Delta#eta(#mu#tau)");
			if (std::string::npos != title1.find("dPhiDil"))  h->GetXaxis()->SetTitle("#Delta#Phi(#mu#tau) [rad]");


			if (std::string::npos != title1.find("Smin"))  h->GetXaxis()->SetTitle("S_{min}^{1/2} [GeV]");
			if (std::string::npos != title1.find("TopMod"))  h->GetXaxis()->SetTitle("Topness [GeV]");
			if (std::string::npos != title1.find("TopMod1L"))  h->GetXaxis()->SetTitle("Topness(1l) [GeV]");
...
//			if (std::string::npos != title1.find("MCTb"))  h->GetXaxis()->SetTitle("M_{CTb} [GeV]");
			...
			if (std::string::npos != title1.find("MTel"))  h->GetXaxis()->SetTitle("M_{T}(e) [GeV]");
			...
			if (std::string::npos != title1.find("MTmuel"))  h->GetXaxis()->SetTitle("M_{T}(e#mu) [GeV]");
			...
			if (std::string::npos != title1.find("dR_muel"))  h->GetXaxis()->SetTitle("#Delta R(e#mu)");
			if (std::string::npos != title1.find("dR_taujet"))  h->GetXaxis()->SetTitle("#Delta R(J_{0}#tau)");
			...
			if (std::string::npos != title1.find("InvMassMuTau"))  h->GetXaxis()->SetTitle("M(#mu#tau) [GeV]");
			if (std::string::npos != title1.find("InvMassElTau"))  h->GetXaxis()->SetTitle("M(e#tau) [GeV]");
			if (std::string::npos != title1.find("InvMassMuEl"))  h->GetXaxis()->SetTitle("M(e#mu) [GeV]");
			...
			if (std::string::npos != title1.find("MeffEl"))  h->GetXaxis()->SetTitle("M_{eff}(eH_{T})");
			if (std::string::npos != title1.find("MeffMu"))  h->GetXaxis()->SetTitle("M_{eff}(#muH_{T})");
			if (std::string::npos != title1.find("MeffTau"))  h->GetXaxis()->SetTitle("M_{eff}(#tauH_{T})");
			if (std::string::npos != title1.find("MeffElOsqrMET"))  h->GetXaxis()->SetTitle("M_{eff}(eH_{T})/#sqrt{#it{p}_{T}^{miss}}");
			if (std::string::npos != title1.find("MeffMuonOsqrMET"))  h->GetXaxis()->SetTitle("M_{eff}(#muH_{T})/#sqrt{#it{p}_{T}^{miss}}");
			if (std::string::npos != title1.find("MeffTauOsqrMET"))  h->GetXaxis()->SetTitle("M_{eff}(#tauH_{T})/#sqrt{#it{p}_{T}^{miss}}");
			...
			if (std::string::npos != title1.find("Mt2High"))  h->GetXaxis()->SetTitle("M_{T2} [GeV]");
			if (std::string::npos != title1.find("Mt2Low"))  h->GetXaxis()->SetTitle("M_{T2} [GeV]");
			if (std::string::npos != title1.find("Mt2Middle"))  h->GetXaxis()->SetTitle("M_{T2} [GeV]");
			if (std::string::npos != title1.find("METLow"))  h->GetXaxis()->SetTitle("#it{p}_{T}^{miss} [GeV]");
			if (std::string::npos != title1.find("METHigh"))  h->GetXaxis()->SetTitle("#it{p}_{T}^{miss} [GeV]");
			if (std::string::npos != title1.find("METMiddle"))  h->GetXaxis()->SetTitle("#it{p}_{T}^{miss} [GeV]");
