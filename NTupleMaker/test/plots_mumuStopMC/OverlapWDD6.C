...
if (SO_)
	...
	/////////////SinglTop
		...
		if (obj->IsA ()->InheritsFrom ("TH1")) {
			...
			//pie3->Draw("3d");
			nan_.erase(0,4);
			sprintf(savename,"TauWDD/Pie%s.pdf",obj->GetName());
...
if (SO_){
			...
			//ratioH->GetXaxis()->SetRange(ratioH->FindFirstBinAbove(0),ratioH->FindLastBinAbove(MaxEventsBin));
      		        if ( string::npos == nn.find("CutFlowUnW") && string::npos == nn.find("1D_")) 
			ratioH->GetXaxis()->SetRange(hh[1]->FindFirstBinAbove(0),hh[1]->FindLastBinAbove(MaxEventsBin));

			//ratioH->GetXaxis()->SetRange(hh[1]->FindFirstBinAbove(0),hh[1]->GetNbinsX());
